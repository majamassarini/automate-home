...
class HasRained(Callable):
    def run(self, event, state):
        if event == home.event.rain.in_the_past.Event.On:
            if home.event.rain.forecast.Event.On in state:
                state = self.get_new_state(state, "off")
        else:
            if (
                home.event.rain.forecast.Event.Off in state
                and home.event.rain.Event.No in state
...
class WillRain(Callable):
    def run(self, event, state):
        if event == home.event.rain.forecast.Event.On:
            if home.event.rain.in_the_past.Event.On in state:
                state = self.get_new_state(state, "off")
        else:
            if (
                home.event.rain.in_the_past.Event.Off in state
                and home.event.rain.Event.No in state
                and home.event.sun.phase.Event.Sunset in state
                and home.event.enable.Event.On in state
            ):
...
class Forced(Callable):
    def run(self, event, state):
        if event == state.forced_enum.Off:
            ...
            state = state.next(state.forced_enum.Not)
