# SPDX-License-Identifier: GPL-3.0-only
#
# automate home devices
#
# Copyright (C) 2021  Maja Massarini

import home
from home.appliance.callable import Callable


class Presence(Callable):
    def run(self, event, state):
        if event == home.event.presence.Event.Off:
            if home.event.courtesy.Event.On not in state:
                state = self.get_new_state(state, "off")
        elif event == home.event.presence.Event.On:
            if home.event.alarm.armed.Event.On in state:
                if home.event.toggle.Event.Off in state:
                    state = self.get_new_state(state, "alarmed_off")
                else:
                    state = self.get_new_state(state, "alarmed_on")
        return state


class Courtesy(Callable):
    def run(self, event, state):
        if event == home.event.courtesy.Event.Off:
            if home.event.presence.Event.On not in state:
                state = self.get_new_state(state, "off")
        elif event == home.event.courtesy.Event.On:
            if home.event.alarm.armed.Event.On in state:
                if home.event.toggle.Event.Off in state:
                    state = self.get_new_state(state, "alarmed_off")
                else:
                    state = self.get_new_state(state, "alarmed_on")
        return state


class Brightness(Callable):
    def run(self, event, state):
        if event == home.event.sun.brightness.Event.Bright:
            if (
                home.event.courtesy.Event.On in state
                or home.event.presence.Event.On in state
            ):
                state = self.get_new_state(state, "off")
        return state


class Armed(Callable):
    def run(self, event, state):
        return state


class Toggle(Callable):
    def run(self, event, state):
        return state


class Forced(Callable):
    def run(self, event, state):
        if event == state.forced_enum.Off:
            state = self.get_new_state(state, "forced_off")
        elif event != state.forced_enum.Not:
            state = state.unforce()
        return state
