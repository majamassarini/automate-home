...
from home.appliance.sound.player.state.fade_in import callable as _callable
...
class State(
    ...
    """
    >>> fading_in = home.appliance.sound.player.state.fade_in.State()
    >>> off = fading_in.next(home.event.elapsed.Event.On)
    >>> off.compute()
    'Off'
    >>> home.event.elapsed.Event.Off in off
    True
    """

    VALUE = "Fade In"

    VOLUME_EVENT = event.fade_in.volume.Event
    PLAYLIST_EVENT = event.fade_in.playlist.Event
